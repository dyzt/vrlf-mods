...
public record ListReport(string RegistrySource, List<ModStatus> Mods, bool VigemInstalled = false,
    bool VirtualGunInstalled = false);
...
public sealed class ModManager
{
    ...
    /// <summary>The menu row: install when absent, otherwise report status.</summary>
    public Task<ActionReport> VirtualGunMenu() =>
        Gun().IsInstalled() ? VirtualGunStatus() : VirtualGunInstall();
